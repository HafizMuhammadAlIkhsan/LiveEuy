...
Jika di komputer lokal belum terpasang JDK 17 atau Maven:
```bash
cd backend
...
Atau menggunakan Docker Compose (otomatis menjalankan PostgreSQL + Backend):
```bash
cd backend
docker compose up -d
```
...
│   └── CorsConfig.java           # Pengaturan CORS untuk Mobile App & Frontend
├── model/
...
- 📋 [API_CONTRACT.md](API_CONTRACT.md): Spesifikasi kontrak endpoint terpadu untuk Web (React) dan Mobile (Flutter).
